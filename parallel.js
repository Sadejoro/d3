...
d3.csv("energy.csv", function(raw_data) {
  // Convert quantitative scales to floats
  data = raw_data.map(function(d) {
    for (var k in d) {
      if (!_.isNaN(raw_data[0][k] - 0) 
      // && k != 'group' && k!= 'sensor' && k!='time'
    ) {
...
function brush() {
  ...
  console.log(d3.select("#search")[0][0].value)
  if (query.length > 0) {
...
function search(selection,str) {
  ...
  return _(selection).filter(function(d) { return pattern.exec(d.sensor); });
}